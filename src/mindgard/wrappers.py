...
class APIModelWrapper(ModelWrapper):
    ...
    def prompt_to_request_payload(self, prompt: str, system_prompt:str) -> dict[str, Any]:
    ...
    def __call__(self, prompt: str, system_prompt:str) -> str:
        request_payload = self.prompt_to_request_payload(prompt,system_prompt)
...
class HuggingFaceWrapper(APIModelWrapper):
    def __init__(self, api_key: str, api_url: str, request_template: str) -> None:
        super().__init__(
            api_url,
...
class OpenAIWrapper(ModelWrapper):
    ...
    def __call__(self, prompt: str, system_prompt:str) -> str:
        #TODO: support "no system prompt" (vs empty)
...
def get_model_wrapper(
    ...
    if preset == 'huggingface':
        ...
        return HuggingFaceWrapper(api_key=api_key, api_url=url, request_template=request_template)
    elif preset == 'openai':
        if not api_key:
