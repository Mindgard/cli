...
def preflight(model_wrapper: ModelWrapper, console: Console, json_out: bool) -> bool:
    ...
    """
    try:
        if not json_out: console.print("[white]Contacting model...")
        for i in range(5):
        ...
        if not json_out: console.print(f"[red]Could not connect to the model! [white](URL: {model_api}, are you sure it's correct?)")
        ...
        if not json_out: console.print(message)
