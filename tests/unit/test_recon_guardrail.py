...
from mindgard.recon.guardrail import GuardrailService, StartReconRequest, GetEventRequest, ReceivedEvent, \
    PushPromptRequest, GuardrailServiceException
...
class TestReconGuardrailServiceStartRecon:
    ...
    def test_get_reconn_events_should_get_selected_events(self):
        ...
        event_request = GetEventRequest(reconn_id=reconn_id, types=["prompt_request", "complete"], access_token=access_token)
    ...
    def test_get_reconn_events_should_raise_error_for_invalid_reconn_id(self):
        events_url = "http://events.url"
        reconn_id = "invalid-recon-id"
        access_token = "<BEARER_TOKEN>"
        event_request = GetEventRequest(reconn_id=reconn_id, types=["prompt_request", "complete"], access_token=access_token)
        guardrail_service = GuardrailService(reconn_url=events_url, get_events_url=events_url, push_events_url=events_url)
    ...
    def test_get_reconn_events_should_raise_error_for_invalid_event_types(self):
        ...
        with pytest.raises(Exception) as exc_info:
            GetEventRequest(reconn_id=reconn_id, types=["prompt_result"], access_token=access_token)
    ...
    def test_push_prompt_results_should_raise_error_for_invalid_request(self):
        ...
        with pytest.raises(Exception) as exc_info:
            with requests_mock.Mocker() as m:
                ...
                push_prompt_request_validated = PushPromptRequest.model_validate(push_prompt_request)
                guardrail_service.push_prompt_results(push_prompt_request_validated)
